...
export async function POST(req: NextRequest) {
  ...
  const pricePerNight = parseFloat(getSetting("price_per_night") ?? "0");
  const cleaningFee = parseFloat(getSetting("cleaning_fee") ?? "0");
  ...
  db.prepare(
    `INSERT INTO bookings
     ...
     VALUES (?, 'site', ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)`
  ).run(bookingId, guestName, guestEmail, guestPhone, checkin, checkout, guestsCount ?? 1, total, cleaningFee, paymentMethod);

  try {
    if (paymentMethod === "mbway") {
      const result = await createMbwayRequest({ bookingId, amount: total, guestPhone });
      db.prepare("UPDATE bookings SET ifthenpay_request_id = ? WHERE id = ?").run(
        result.RequestId ?? "",
        bookingId
      );
